...
// Лаб. работа №4, задание 1, вариант 6.
...
int main()
{
    ...
    for (float i = x1; i <= x2; i += step)
    {
        ...
        // (к примеру в отрезке [-0.3; 2] с шагом 0.1, не выполняется последняя итерация при x = 2, хотя он входит в отрезок, последняя итерации при x = 1.9
        // В 4-ой по счёту итерации, когда x = -0.1 переходит в 0, в этом нуле появляется какой-то мусор.
